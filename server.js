...
app.post("/api/mining/start", async (req, res) => {
  try {
    ...
    await pool.query(
      "UPDATE users SET mining_started_at=COALESCE(mining_started_at,NOW()) WHERE telegram_id=$1",
      [u.id]
    );
  ...
  try {
    ...
    try {
      ...
      const r = await c.query(
        "SELECT balance, mining_started_at FROM users WHERE telegram_id=$1 FOR UPDATE",
        [u.id]
      );

      const x = r.rows[0];

      if (x?.mining_started_at) {
        const sec =
          (Date.now() - new Date(x.mining_started_at)) / 1000;

        await c.query(
          "UPDATE users SET balance=balance+$1, mining_started_at=NULL WHERE telegram_id=$2",
          [Math.max(0, sec) * 0.25 / 3600, u.id]
        );
      }
...
app.post("/api/tasks/:id/claim", async (req, res) => {
  try {
    const u = await auth(req);
    const reward = TASKS[req.params.id];
    ...
    try {
      ...
      const old = await c.query(
        "SELECT 1 FROM task_claims WHERE telegram_id=$1 AND task_id=$2",
        [u.id, req.params.id]
      );
      ...
      await c.query(
        "INSERT INTO task_claims VALUES($1,$2)",
        [u.id, req.params.id]
      );
